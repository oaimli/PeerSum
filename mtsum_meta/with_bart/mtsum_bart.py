...
class AcceptancePredictor(nn.Module):
    ...
    def forward(self, decoder_last_hidden_state, decoder_input_ids):
        ...
        logits = self.linear_relu_stack(torch.mean(masked_hidden_states, dim=1))
...
class ConfidencePredictor(nn.Module):
    ...
    def forward(self, encoder_last_hidden_state):
        predicted_confidences = self.linear_relu_stack(encoder_last_hidden_state)
...
class RatingPredictor(nn.Module):
    ...
    def forward(self, encoder_last_hidden_state):
        predicted_ratings = self.linear_relu_stack(encoder_last_hidden_state)
...
class DocumentTypePredictor(nn.Module):
    def __init__(self, hidden_size):
        ...
        self.linear_relu_stack = nn.Sequential(
            nn.Linear(hidden_size, 7),# seven classes, including paper_abstract, official_reviewe, official response, public comment, public response and author general response, author response
        )

    def forward(self, encoder_last_hidden_state):
        logits = self.linear_relu_stack(encoder_last_hidden_state)
...
class MTSummarizer(pl.LightningModule):
    def __init__(self, args):
        ...
        self.tokenizer = BartTokenizer.from_pretrained(args.pretrained_model)
        self.model_config = BartConfig()
        self.model_config.max_position_embeddings = args.max_length_input
        self.model = BartForConditionalGeneration.from_pretrained(args.pretrained_model, config=self.model_config, ignore_mismatched_sizes=True)
        ...
        # there is no doc-sep in BART
        self.docsep_token = "<doc-sep>"
        self.tokenizer.add_tokens([self.docsep_token], special_tokens=True)
    ...
    def forward(self, input_ids, output_ids, ratings, confidences):
    ...
    def shared_step(self, input_ids, output_ids, acceptances, confidences, ratings, review_roles):
        ...
        lm_logits, predicted_acceptances, predicted_confidences, predicted_ratings, predicted_review_roles = self.forward(input_ids, output_ids, ratings, confidences)
        ...
        for i in range(batch_size_current):
            ...
            target_indexes = torch.nonzero(input_ids_sample==self.docsep_token_id).squeeze()
            loss_document_type_prediction += cross_entropy_loss_document_type(torch.index_select(predicted_review_roles_sample, 0, target_indexes), torch.index_select(review_roles_sample, 0, target_indexes))
    ...
    def training_step(self, batch, batch_idx):
        input_ids, output_ids, acceptances, confidences, ratings, review_roles = batch
        loss_generation, loss_acceptance_prediction, loss_confidence_prediction, loss_rating_prediction, loss_document_type_prediction = self.shared_step(input_ids, output_ids, acceptances, confidences, ratings, review_roles)
    ...
    def compute_rouge_batch(self, input_ids, gold_str, batch_idx):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        input_ids, output_ids, tgt, acceptances, confidences, ratings, review_roles = batch
        loss_generation, loss_acceptance_prediction, loss_confidence_prediction, loss_rating_prediction, loss_review_role_prediction = self.shared_step(
            input_ids, output_ids, acceptances, confidences, ratings, review_roles)
        loss = loss_generation + loss_acceptance_prediction + loss_confidence_prediction + loss_rating_prediction + loss_review_role_prediction
        result_batch = self.compute_rouge_batch(input_ids, tgt, batch_idx)
        return {"validation_loss": loss, "validation_loss_generation": loss_generation, "validation_loss_acceptance": loss_acceptance_prediction, "validation_loss_confidence": loss_confidence_prediction, "validation_loss_rating": loss_rating_prediction, "validation_loss_review_role": loss_review_role_prediction, "rouge_result": result_batch}
    ...
    def test_step(self, batch, batch_idx):
        ...
        loss = loss_generation + loss_acceptance_prediction + loss_confidence_prediction + loss_rating_prediction + loss_review_role_prediction
        result_batch = self.compute_rouge_batch(input_ids, tgt, batch_idx)
        return {"test_loss": loss, "test_loss_generation": loss_generation, "test_loss_acceptance":loss_acceptance_prediction, "test_loss_confidence": loss_confidence_prediction, "test_loss_rating": loss_rating_prediction, "test_loss_review_role": loss_review_role_prediction, "rouge_result": result_batch}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--length_penalty", type=float, default=0.6, help="length penalty of generated text")
