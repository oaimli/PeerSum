...
from dataloader_pegasus import get_dataloader_summ
...
class MTSummarizer(pl.LightningModule):
    def __init__(self, args):
        ...
        self.tokenizer = PegasusTokenizer.from_pretrained(args.pretrained_model)
        # self.config = PegasusConfig.from_pretrained(args.pretrained_model)
        # self.config.max_position_embeddings = args.max_length_input
        # self.model = PegasusForConditionalGeneration.from_pretrained(args.pretrained_model, config=self.config)
        self.model = PegasusForConditionalGeneration.from_pretrained(args.pretrained_model)
    ...
    def forward(self, input_ids, document_startings, document_endings, document_levels, document_relationship_matrices,
        ...
        """
        document_startings: (batch_size, seq_len)
        document_endings: (batch_size, seq_len)
        document_levels: batch_size, document_levels for one sample
        document_relationship_matrices: batch_size, relationships, a relationship matrix of (document_count, document_count) for each relationship
        """
    ...
    def shared_step(self, input_ids, document_startings, document_endings, document_levels,
        ...
        # return loss_generation, torch.tensor(0.0), torch.tensor(0.0), torch.tensor(0.0), torch.tensor(0.0)
