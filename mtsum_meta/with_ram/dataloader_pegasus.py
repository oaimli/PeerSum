...
class SummarizationDataset(Dataset):
    def __init__(
        ...
        # self.docsep_token_id = self.tokenizer.
        self.docsep_token_id = self.tokenizer.eos_token_id
    ...
    def __getitem__(self, idx):
        ...
        for source_document in source_documents:
            ...
            document_startings.append(len(input_ids))
            ...
            document_endings.append(len(input_ids)-1)
        ...
        # document_types = (document_types + [-1])
        ...
        if self.dataset_type == "train":
            ...
            return torch.tensor(input_ids, dtype=torch.long), torch.tensor(document_startings, dtype=torch.long), torch.tensor(
                document_endings, dtype=torch.long), torch.tensor(document_levels, dtype=torch.long), torch.tensor(
                document_relationship_matrices, dtype=torch.float), torch.tensor(output_ids, dtype=torch.long), acceptance, torch.tensor(
                confidences, dtype=torch.float), torch.tensor(ratings, dtype=torch.float), torch.tensor(document_types, dtype=torch.long), tgt
