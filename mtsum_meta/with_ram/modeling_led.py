...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    ...
    # expanded_mask: 0, no attn; local attn: 1; global attn: 2

    inverted_mask = 1.0 - expanded_mask
    # inverted_mask: no attn, 1.0; local attn: 0.0; global attn: -1.0
...
# generate different cross-attention masks for different layers, based on _expand_mask
def _expand_mask_layers(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None, document_startings=None,
                        document_endings=None, document_levels=None, num_layers: int = None):
    """
    Expands attention_mask from `[bsz, seq_len]` to `[layers, bsz, 1, tgt_seq_len, src_seq_len]`.
    """
    # pdb.set_trace()
    bsz, src_len = mask.size()
    tgt_len = tgt_len if tgt_len is not None else src_len
    ...
    for layer_id in range(num_layers):
        # only make modification on all layers except for the last layer
        if layer_id < num_layers - 1:
            for sample_id, document_levels_sample in enumerate(document_levels):
                document_startings_sample = document_startings[sample_id]
                document_endings_sample = document_endings[sample_id]
                seq_mask = mask[sample_id]

                for document_id, document_level in enumerate(document_levels_sample):
                    if document_level - 1 != layer_id:
                        document_starting = document_startings_sample[document_id]
                        document_ending = document_endings_sample[document_id]
                        seq_mask[document_starting:document_ending + 1] = 0
                mask[sample_id] = seq_mask
        ...
        # make sure that global_attn_mask is positive
        expanded_attention_mask = expanded_attention_mask * inverted_mask
        expanded_attention_masks += (expanded_attention_mask,)

    return expanded_attention_masks
...
# Relationship-aware Sparse Attention for the LEDEncoderLayer, based on LEDEncoderSelfAttention
# LEDEncoderSelfAttention will no longer be used
class LEDEncoderRelationshipAwareSparseAttention(nn.Module):
    def __init__(self, config, layer_id):
        ...
        self.num_relationships = 7
        self.relationship_weight_matrix = nn.Parameter(
            torch.rand((self.num_heads, self.num_relationships), requires_grad=True))
        torch.nn.init.normal_(self.relationship_weight_matrix)
    ...
    def forward(
            ...
            document_startings=None,
            document_endings=None,
            document_relationship_matrices=None
    ):
        """
        ...
        *attention_window* happens in [`LEDEncoderModel.forward`] to avoid redoing the padding on each layer.
        ...
        # layer_head_mask, None
        # is_index_masked, (batch_size, seq_len)
        # is_index_global_attn, (batch_size, seq_len)
        # is_global_attn, True


        # pdb.set_trace()
        hidden_states = hidden_states.transpose(0, 1)
        # hidden_stats, (seq_len, batch_size, hidden_size)

        # project hidden states
        query_vectors = self.query(hidden_states)
        # query_vectors, (seq_len, batch_size, embed_dim)
        key_vectors = self.key(hidden_states)
        # key_vectors, (seq_len, batch_size, embed_dim)
        value_vectors = self.value(hidden_states)
        # value_vectors, (seq_len, batch_size, embed_dim)
        ...
        query_vectors = query_vectors.view(seq_len, batch_size, self.num_heads, self.head_dim).permute(1, 2, 0, 3)
        # query_vectors, (1, num_heads, seq_len , head_dim)
        key_vectors = key_vectors.view(seq_len, batch_size, self.num_heads, self.head_dim).permute(1, 2, 0, 3)
        # key_vectors, (1, num_heads, seq_len, head_dim)
        value_vectors = value_vectors.view(seq_len, batch_size, self.num_heads, self.head_dim).permute(1, 2, 0, 3)
        # value_vectors, (1, num_heads, seq_len, head_dim)

        query_vectors = torch.squeeze(query_vectors, dim=0)
        # query_vectors, (num_heads, seq_len, head_dim)
        key_vectors = torch.squeeze(key_vectors, dim=0)
        # key_vectors, (num_heads, seq_len, head_dim)
        value_vectors = torch.squeeze(value_vectors, dim=0)
        # query_vectors, (num_heads, seq_len, head_dim)


        # # directly multiplication for all positions
        # attention_weights = torch.matmul(query_vectors, key_vectors.transpose(1, 2))
        # expanded_mask = attention_mask[:, None, None, :].expand(batch_size, 1, seq_len, seq_len).to(attention_weights.dtype)
        # expanded_mask = expanded_mask.reshape(batch_size * 1, seq_len, seq_len)
        # print(expanded_mask)
        # attention_weights = attention_weights.masked_fill(expanded_mask < 0, torch.torch.finfo(attention_weights.dtype).min)
        # attention_weights = nn.functional.softmax(attention_weights, dim=-1)
        # attention_weights = nn.functional.dropout(attention_weights, p=self.dropout, training=self.training)
        # attention_output = torch.matmul(attention_weights, value_vectors)
        # attention_output = torch.unsqueeze(attention_output, 0).contiguous()
        # attention_output = attention_output.view(batch_size, seq_len, -1)


        # relationship-aware attention for all positions
        # assume that the batch_size is 1
        document_starting = document_startings[0]
        document_ending = document_endings[0]
        document_relationship_matrix = document_relationship_matrices[0]
        attention_mask_sample = attention_mask[:, None, None, :].expand(batch_size, 1, seq_len, seq_len).to(document_starting.dtype)[0]
        # attention_mask_sample, (1, seq_len, seq_len)

        attention_output = torch.zeros_like(value_vectors)
        # attention_output, (bsz*num_heads, seq_len, head_dim)

        # assume that the batch_size is 1
        for head_index in range(self.num_heads):
            # print(head_index)
            query_vectors_head = query_vectors[head_index]
            # query_vectors_head, (seq_len, head_dim)
            key_vectors_head = key_vectors[head_index]
            # key_vectors_head, (seq_len, head_dim)
            key_vectors_head_transpose = key_vectors_head.transpose(0, 1)
            # key_vectors_head_transpose, (head_dim, seq_len)
            value_vectors_head = value_vectors[head_index]
            # value_vectors_head, (seq_len, head_dim)


            # # using attention mask based on padding
            # attention_weights = torch.matmul(query_vectors_head, key_vectors_head_transpose)
            # attention_weights = attention_weights + attention_mask_sample[0]
            # attention_weights = nn.functional.softmax(attention_weights, dim=-1)
            # attention_weights = nn.functional.dropout(attention_weights, p=self.dropout, training=self.training)
            # head_output = torch.matmul(attention_weights, value_vectors_head)


            ######################
            # using relationship-aware attention

            # # different heads have single different relationships
            # relationship_index = int(head_index % document_relationship_matrix.size()[0])
            # relationship_matrix = document_relationship_matrix[relationship_index]

            # each head has a weighted sum of different relationships
            relationship_weight_vector = self.relationship_weight_matrix[head_index]
            # print(relationship_weight_vector.size())
            _, num_documents, _ = document_relationship_matrix.size()
            relationship_matrix = torch.matmul(relationship_weight_vector.reshape(1, self.num_relationships),
                                               document_relationship_matrix.reshape(self.num_relationships, -1))
            relationship_matrix = relationship_matrix.reshape(num_documents, num_documents)

            head_output = torch.zeros_like(value_vectors_head)
            # print(relationship_matrix.size(), document_starting.size())
            for document_index in range(document_starting.size()[0]):
                starting = document_starting[document_index]
                ending = document_ending[document_index]

                query_vectors_head_document = query_vectors_head[starting:ending + 1]
                document_attn_weights = torch.matmul(query_vectors_head_document, key_vectors_head_transpose)

                # using relationship-aware mask
                document_attn_mask = torch.zeros_like(document_attn_weights)
                document_relationship_vector = relationship_matrix[document_index]
                # print("head_%d"%head_index, "document_%d"%document_index, document_relationship_vector)
                # print(document_starting)
                for document_relationship_index in range(document_relationship_vector.size()[0]):
                    if document_relationship_vector[document_relationship_index] > 0:
                        other_starting = document_starting[document_relationship_index]
                        other_ending = document_ending[document_relationship_index]
                        # print(other_starting, other_ending)
                        document_attn_mask[:][other_starting:other_ending + 1] = document_relationship_vector[
                            document_relationship_index]
                        # document_attn_mask[:][other_starting:other_ending + 1] = 1
                document_attn_weights = torch.mul(document_attn_weights, document_attn_mask)
                document_attn_weights = document_attn_weights.masked_fill(document_attn_mask == 0, torch.torch.finfo(
                    document_attn_weights.dtype).min)

                # # using padding based mask
                # attention_mask_sample_document = attention_mask_sample[0][starting:ending+1]
                # document_attn_weights = document_attn_weights + attention_mask_sample_document

                document_attn_probs = nn.functional.softmax(document_attn_weights, dim=-1)
                document_attn_probs = nn.functional.dropout(document_attn_probs, p=self.dropout, training=self.training)
                head_output[starting:ending + 1] = torch.matmul(document_attn_probs, value_vectors_head)
            ######################

            attention_output[head_index] = head_output
        ...
        assert attention_output.size() == (batch_size, seq_len,
                                           self.config.hidden_size), f"Attention output should be of size {(batch_size, seq_len, self.config.hidden_size)}, but is {attention_output.size()}"


        # copied from the original code, compute value for global attention and overwrite to attention output
        # attention_output needs to be (seq_len, batch_size, embed_dim)
        ...
        if is_global_attn:
            ...
            attention_output[is_index_global_attn_nonzero[::-1]] = nonzero_global_attn_output.view(
                len(is_local_index_global_attn_nonzero[0]), -1
            )

            # global_attn_probs = torch.zeros((batch_size, self.num_heads, max_num_global_attn_indices, seq_len)).type_as(
            #     hidden_states)
        attention_output = attention_output.transpose(0, 1)

        outputs = (attention_output,)
        # outputs[0] is required to be of the size (batch_size, seq_len, hidden_size)
        # global_attn_probs, (batch_size, heads, layers, seq_len)
        ...
        # #######
        # # below is the original code of longformer sparse attention
        # #######
        #
        # hidden_states = hidden_states.transpose(0, 1)
        # # hidden_stats, (seq_len, batch_size, hidden_size)
        ...
        # pdb.set_trace()
        # outputs[0], (batch_size, seq_len, hidden_size)
        # global_attn_probs, (batch_size, heads, layers, seq_len)
...
class LEDEncoderAttention(nn.Module):
    def __init__(self, config, layer_id):
        super().__init__()
        self.longformer_self_attn = LEDEncoderRelationshipAwareSparseAttention(config, layer_id=layer_id)
...
# LEDDecoderRelationshipAwareCrossAttention for the LEDDecoder cross attentions, based on LEDDecoderAttention
# LEDDecoderAttention will only be used in LEDDecoder self-attentions
# This is the same as LEDDecoderAttention, and there are no changes,
# because no need to modify this, only construct different attention masks in LEDDecoder is enough
class LEDDecoderRelationshipAwareCrossAttention(nn.Module):
...
class LEDDecoderAttention(nn.Module):
    """
    Multi-headed attention from 'Attention Is All You Need' paper
    This will be only used for LEDDecoder self-attentions
    """
...
class LEDDecoderLayer(nn.Module):
    def __init__(self, config: LEDConfig):
        ...
        self.self_attn_layer_norm = nn.LayerNorm(self.embed_dim)
        self.encoder_attn = LEDDecoderRelationshipAwareCrossAttention(
            self.embed_dim,
    ...
    def forward(
        ...
        Args:
            ...
            return_dict (`bool`, *optional*):
                Whether or not to return a [`~utils.ModelOutput`] instead of a plain tuple.
        """
        # print("document startings", document_startings)
        # print("docuemnt endings", document_endings)
                ...
                if self.gradient_checkpointing and self.training:
                    ...
                    def create_custom_forward(module):
                        def custom_forward(*inputs):
                            return module(*inputs, is_global_attn, output_attentions, document_startings,
                                          document_endings, document_relationship_matrices)
...
class LEDDecoder(LEDPreTrainedModel):
    ...
    def forward(
        ...
        if encoder_hidden_states is not None and encoder_attention_mask is not None:
            ...
            # # construct different attention masks for different layers
            # encoder_attention_masks = _expand_mask_layers(encoder_attention_mask, inputs_embeds.dtype,
            #                                               tgt_len=input_shape[-1],
            #                                               document_startings=document_startings,
            #                                               document_endings=document_endings,
            #                                               document_levels=document_levels, num_layers=self.num_layers)
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if encoder_attention_masks != None:
                encoder_attention_mask = encoder_attention_masks[idx]
...
class LEDModel(LEDPreTrainedModel):
    ...
    def forward(
            ...
            document_startings: Optional[Tuple[torch.FloatTensor]] = None,
            document_endings: Optional[Tuple[torch.FloatTensor]] = None,
            decoder_document_levels: Optional[Tuple[torch.FloatTensor]] = None,
            document_relationship_matrices: Optional[Tuple[torch.FloatTensor]] = None
        ...
        decoder_outputs = self.decoder(
            ...
            document_startings=document_startings,
            document_endings=document_endings,
            document_levels=decoder_document_levels
        )
...
class LEDForConditionalGeneration(LEDPreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.led(
            ...
            document_startings=document_startings,
            document_endings=document_endings,
            decoder_document_levels=decoder_document_levels,
            document_relationship_matrices=document_relationship_matrices
        )
