    ...
    if split_name == "test":
        with jsonlines.open(args.data_path + '%s.json' % args.dataset_name) as reader:
            for line in reader:
                if line["label"] == "test":
                    dataset.append(line)
        if len(dataset) > args.num_test_data > 0 and args.test_hard_cases == False:
            random.seed(42)
            dataset = random.sample(dataset, k=args.num_test_data)
        elif args.test_hard_cases == True:
            for paper in dataset[:]:
                ...
                if len(source_documents) <= 15 or with_disagreements == False:
                    dataset.remove(paper)
