...
from dataloader_bart import get_dataloader_summ
...
class MTSummarizer(pl.LightningModule):
    def __init__(self, args):
        ...
        self.tokenizer = BartTokenizerFast.from_pretrained(args.pretrained_model)
        # self.config = BartConfig.from_pretrained(args.pretrained_model)
        # self.config.max_position_embeddings = args.max_length_input
        config = BartConfig()
        self.model = BartForConditionalGeneration(config=config)
        self.model = BartForConditionalGeneration.from_pretrained(args.pretrained_model)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_hard_cases", action="store_true", help="Whether test only on hard cases")
