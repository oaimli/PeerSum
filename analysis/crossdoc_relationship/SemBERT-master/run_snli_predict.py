# coding=utf-8
"""
CUDA_VISIBLE_DEVICES=0 python run_snli_predict.py --data_dir glue_data/SNLI --task_name snli --eval_batch_size 128 --max_seq_length 128 --max_num_aspect 3 --do_predict --do_lower_case --bert_model snli_model_dir --output_dir snli_model_dir --tagger_path srl_model_dir
"""
...
def main():
    ...
    if args.do_predict:
        ...
        output_test_file = os.path.join(args.output_dir, "SNLI_pred_results.tsv")
        index = 0
        ...
        for pred in predictions:
            if int(pred)==0:
                contra_count += 1
        print("contradictory ratio", contra_count/len(predictions))

        errors = 0
        for index, example in enumerate(eval_examples):
            if example.label!=label_list[int(predictions[index])]:
                errors += 1
        print("accuracy", 1-errors/len(eval_examples))
